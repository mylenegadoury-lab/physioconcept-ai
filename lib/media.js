...
export async function findStockImage(exerciseName, prompt) {
  try {
    ...
    console.log(`🔍 findStockImage appelé pour: "${exerciseName}" avec query: "${query}"`);

    // 1) Pexels (requiert PEXELS_API_KEY dans .env.local)
    if (process.env.PEXELS_API_KEY) {
      console.log("🔑 Tentative Pexels avec API key");
      try {
        ...
        if (res.ok) {
          ...
          if (j.photos && j.photos.length > 0) {
            ...
            console.log(`✅ Pexels image trouvée: ${imageUrl.substring(0, 60)}...`);
            return imageUrl;
          }
        }
      } catch (pexErr) {
    ...
    console.log(`📸 Utilisation Unsplash fallback: ${unsplashUrl}`);
    return unsplashUrl;
  } catch (err) {
