 ...
 * ENHANCED: Intègre système éducatif complet
 */

import OpenAI from 'openai';
import { generateComprehensiveEducation } from './educationEngine.js';
 ...
 * ENHANCED: Utilise l'analyse clinique approfondie + système éducatif complet
 */
export async function generateEnrichedProgram({ selectedExercises, patientProfile, justifications, clinicalAnalysis }) {
  console.log('\n🎨 PROGRAMME ENRICHMENT - Starting with clinical analysis + education...\n');

  // Build enhanced context with clinical analysis
  const context = buildPatientContext(patientProfile, clinicalAnalysis);
  
  // Generate comprehensive patient education
  const comprehensiveEducation = generateComprehensiveEducation(patientProfile, clinicalAnalysis);
  console.log('   ✅ Education system generated');
  
  // Enrich each exercise with clinical insights
  const enrichedExercises = await Promise.all(
    selectedExercises.map((exercise, index) => 
      enrichExercise(exercise, patientProfile, context, justifications[index], clinicalAnalysis, comprehensiveEducation)
    )
  );

  // Generate global education and recommendations
  const globalGuidance = await generateGlobalGuidance(patientProfile, enrichedExercises, clinicalAnalysis, comprehensiveEducation);
  ...
  return {
    ...
    comprehensiveEducation: comprehensiveEducation, // NEW: Include education
 ...
 * Inclut éducation contextualisée
 */
async function enrichExercise(exercise, patientProfile, context, justification, clinicalAnalysis, comprehensiveEducation) {
  console.log(`   🎯 Enriching: ${exercise.name}...`);

  const isProfessional = patientProfile.isProfessional === true;

  // Enhanced prompts with clinical analysis + education
  const prompt = isProfessional 
    ? getProfessionalPrompt(exercise, context, justification, clinicalAnalysis, comprehensiveEducation) 
    : getPatientPrompt(exercise, context, justification, clinicalAnalysis, comprehensiveEducation);
...
/**
 * Prompt pour MODE PATIENT (simplifié, accessible) - ENHANCED with education
 */
function getPatientPrompt(exercise, context, justification, clinicalAnalysis, comprehensiveEducation) {
  ...
  // Educational context
  const beliefs = comprehensiveEducation?.beliefs;
  const keyEducation = comprehensiveEducation?.education?.phenotype_specific;
  
  let educationalContext = '';
  if (keyEducation) {
    educationalContext = `\nCONTEXTE ÉDUCATIF IMPORTANT:
- Type de lombalgie: ${keyEducation.title}
- Pourquoi ça marche: ${keyEducation.why_exercises_work || keyEducation.explanation}`;
  }
  
  if (beliefs && beliefs.priority.length > 0) {
    educationalContext += `\n- Message éducatif clé: ${beliefs.priority[0]}`;
  }
...
/**
 * Prompt pour MODE PROFESSIONNEL (détaillé, clinique) - ENHANCED with education
 */
function getProfessionalPrompt(exercise, context, justification, clinicalAnalysis, comprehensiveEducation) {
  ...
  // Educational insights for clinical reasoning
  const beliefs = comprehensiveEducation?.beliefs;
  const education = comprehensiveEducation?.education;
...
`;
  ...
  if (phenotype) {
    ...
    if (primary) {
      ...
      if (primary.implications) {
        ...
        clinicalContext += `   - Progression: ${primary.implications.progression}\n`;
      }
    }
  }
  ...
  // Add educational context for clinicians
  if (beliefs && beliefs.identified.length > 0) {
    clinicalContext += `\n6. CROYANCES & FACTEURS PSYCHOSOCIAUX IDENTIFIÉS:\n`;
    beliefs.identified.slice(0, 3).forEach(b => {
      clinicalContext += `   - ${b.belief} (${b.severity}): ${b.description}\n`;
    });
    if (beliefs.priority.length > 0) {
      clinicalContext += `   PRIORITÉ ÉDUCATIVE: ${beliefs.priority[0]}\n`;
    }
  }
  
  if (education?.prognosis) {
    clinicalContext += `\n7. PRONOSTIC:\n`;
    clinicalContext += `   - Overall: ${education.prognosis.overall}\n`;
    clinicalContext += `   - Success rate: ${education.prognosis.success_rate}\n`;
    clinicalContext += `   - Timeline: ${education.prognosis.timeline}\n`;
  }
